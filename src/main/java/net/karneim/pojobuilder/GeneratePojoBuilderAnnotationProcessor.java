...
public class GeneratePojoBuilderAnnotationProcessor extends AbstractProcessor {
	...
	private static final Logger LOG = Logger
			.getLogger(GeneratePojoBuilderAnnotationProcessor.class.getName());
	...
	public void process(TypeElement productTypeElem) {
		String productSimpleName = TypeUtil.getSimpleName(productTypeElem
				.getQualifiedName().toString());

		TypeM builderBaseclass = getBuilderBaseclass(productTypeElem);
		String builderPackageName = getBuilderPackage(productTypeElem);
		String builderClassname = getBuilderClassname(builderPackageName,
				productSimpleName);

		TypeM productType = TypeUtil.getTypeM(productTypeElem);
		...
		try {
			BuilderM model = new BuilderMBuilder()
					...
					.withProperties(
							findProperties(productTypeElem, builderPackageName))
					.build();
			...
			processingEnv.getMessager().printMessage(Diagnostic.Kind.ERROR,
					String.format("Error while processing: %s", e));
			throw new UndeclaredThrowableException(e);
		}
	}

	private String getBuilderClassname(String packageName,
			String productSimpleName) {
	...
	private PackageElement findPackage(TypeElement typeElement) {
		...
		if (tmp instanceof PackageElement) {
			...
			throw new IllegalStateException(String.format(
					"Can't find enclosing package of element %s", typeElement));
	}
	...
	private Collection<PropertyM> findProperties(TypeElement productTypeElem,
		...
		while (current != null) {
			...
			propertyFinder.setVisitConstructors(current == productTypeElem);
